...
const lower=sentence.replace(/[^a-zA-Z\s]/g,'').split(" ").filter(char=>char[0]===char[0].toLowerCase()).sort().join(" ")
const high=sentence.replace(/[^a-zA-Z\s]/g,'').split(" ").filter(char=>char[0]===char[0].toUpperCase() ).sort((a,b)=>b.localeCompare(a)).join(" ")
return lower && high?`${lower} ${high}`:lower?lower:high
}