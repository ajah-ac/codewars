...
For example, consider the goal of 6, and big=1, small=3. Using the existing one big chocolate, it is not possible to achieve 
the remainder of the weight of 1. Therefore, avoid using the big chocolate. Use the existing 3 small chocolates and achieve the goal.
...
The function should return the number of small chocolates required to achieve the goal. The function should return -1 only if the goal
 cannot be achieved by any possible combination of big chocolates and small chocolates.
...
function makeChocolates(small, big, goal) {
  for (let i = Math.min(big, Math.floor(goal / 5)); i >= 0; i--) {
    const remain = goal - i * 5;
    if (remain % 2 === 0 && remain / 2 <= small) return remain / 2;
  }
  return -1;
}
