...
this.items=items;
    ...
    vend(selection, itemMoney){
        const selected= this.items.find(item=>item.code===selection)
        if(!selected) return `Invalid selection! : Money in vending machine = ${this.money.toFixed(2)}`
        if(selected.quantity<=0)return `${selected.name}: Out of stock!`
        if(itemMoney===selected.price) {
          selected.quantity--
          this.money+=selected.price
          return `Vending ${selected.name}`}
        if(itemMoney-selected.price<0) return `Not enough money!`
        if(itemMoney-selected.price>0) {
                    selected.quantity--

                    this.money+=selected.price
      return `Vending ${selected.name} with ${(itemMoney-selected.price).toFixed(2)} change.`}
